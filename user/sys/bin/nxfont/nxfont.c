 ...
 *
 * After the upload, nxfont spins forever (while(1) yield()) because the
 * kernel's sys_set_font implementation stores a raw pointer into userland
 * heap memory rather than copying the data.  If nxfont exited, the kernel
 ...
 *                  process's heap; nxfont cannot exit.  Fix: kernel must
 ...
 *   USR-BLOAT-01/02 (W2 BAD-IMPL·PERF) nxfont.elf is ~773KB (verified),
 ...
 * wrapping operations.
 */
static double fmod(double x, double y) { return x - (int)(x / y) * y; }
 ...
 *   final_data in userland heap memory.  nxfont must not exit; if it did
...
int main(int argc, char **argv) {
  ...
  struct font_glyph_info *glyphs =
      malloc(sizeof(struct font_glyph_info) * num_chars);
  ...
  for (int i = 0; i < num_chars; i++) {
    ...
    unsigned char *pixels = stbtt_GetCodepointBitmap(&font, 0, scale, codepoint,
                                                     &w, &h, &xoff, &yoff);
  ...
  header.descent =
      (uint16_t)(-descent * scale); /* descent is negative in font units */
  ...
  size_t total_size = sizeof(header) +
                      (sizeof(struct font_glyph_info) * num_chars) +
                      bitmap_offset;
  ...
  ptr += sizeof(header);
  memcpy(ptr, glyphs, sizeof(struct font_glyph_info) * num_chars);
  ptr += sizeof(struct font_glyph_info) * num_chars;
  ...
  /* Keep it in memory if the kernel just points to it (for now, as per
   * sys_set_font hack). NOTE(USR-FONTMAN-01): This spin loop is mandatory under
   * the current kernel implementation.  The kernel's sys_set_font stores a raw
   * pointer into final_data; exiting nxfont would free it and leave the kernel
   * with a dangling pointer, causing a use-after-free on every subsequent text
   * render. Fix: kernel should copy the buffer at SYS_SET_FONT time. */
  while (1)
    yield();
