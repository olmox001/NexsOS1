...
/*
 * create_utmp_file - crea /etc/utmp con un record di login per root.
 * Necessario per `who`, `logname` e altri comandi che leggono utmp.
 */
...
/*
 * struct nx_timeval — mini-def locale per la struct utmp.
 * Il progetto non ha un <sys/time.h> che definisca struct timeval;
 * time_t è già definito in posix_types.h come int64_t.
 */
struct nx_timeval {
    time_t tv_sec;
    long   tv_usec;
...
struct utmp {
    ...
    long ut_session;
    struct nx_timeval ut_tv;       /* era struct timeval — tipo incompleto */
    int32_t ut_addr_v6[4];
    char ut_pad[20];               /* era __unused — conflitto con macro in posix_types.h */
};

/*
 * nx_mkdir_p — crea ricorsivamente una directory e i suoi genitori.
 *
 * SYS_MKDIR è stato ritirato da Programme R1b (syscall_nums.h:118-121);
 * la creazione di directory avviene tramite OBJ_CTL_MKDIR su un handle
 * MUTATE alla directory genitore.  Questa helper incapsula la logica
 * di split path + handle_create + object_ctl + close.
 */
static void nx_mkdir_p(const char *path) {
    ...
    /* Scansiona i componenti dal fondo e crea dal primo che manca */
    for (char *p = buf + 1; *p; p++) {
        if (*p == '/') {
            *p = '\0';
            int h = OS1low_handle_create(OS1_NS_FS, buf, OS1_RIGHT_MUTATE, OBJ_TYPE_FILE);
            if (h >= 0) {
    ...
    if (slash && slash[1]) {
        ...
        parent[plen] = '\0';
        int h = OS1low_handle_create(OS1_NS_FS, parent, OS1_RIGHT_MUTATE, OBJ_TYPE_FILE);
        if (h >= 0) {
...
static void create_utmp_file(void) {
    struct utmp rec;
    memset(&rec, 0, sizeof(rec));

    rec.ut_type = 7;          /* USER_PROCESS */
    rec.ut_pid = 1;           /* init PID (o qualsiasi) */
    strncpy(rec.ut_line, "tty1", sizeof(rec.ut_line) - 1);
    strncpy(rec.ut_id, "tty1", sizeof(rec.ut_id) - 1);
    strncpy(rec.ut_user, "root", sizeof(rec.ut_user) - 1);
    rec.ut_tv.tv_sec = time(NULL);
    rec.ut_tv.tv_usec = 0;

    /* Crea le directory se non esistono (via handle API, non mkdir syscall) */
    nx_mkdir_p("/etc");

    if (file_write("/etc/utmp", &rec, sizeof(rec), 0) < 0) {
        printf("[Init] WARN: could not create /etc/utmp\n");
    } else {
        printf("[Init] Created /etc/utmp with root login record\n");
    }
}
...
int main(void) {
    ...
    /* Crea il file /etc/utmp per supportare `who`, `logname`, ecc. */
    create_utmp_file();
