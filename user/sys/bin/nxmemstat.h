 ...
 *
 * Reusable header-only helper (all routines `static inline`) over the privileged
 * OS1_sys_stats() syscall, mirroring nxproc.h.  SECURITY MODEL: no ambient
 * checks of its own — OS1_sys_stats() is gated to ROOT/machine in the kernel, so
 * only an NX service (this runs as /sys/bin/nxmemstat at ROOT) can read the raw
 * stats; USER apps consume the service's GUI, never the syscall.
 *
 * Two surfaces:
 *   - GUI: nxmemstat_render_if_changed() draws a compositor WINDOW (virtio UI),
 *     redrawing only when a MEANINGFUL value changed (the signature deliberately
 *     EXCLUDES the always-incrementing cumulative counters — ctx switches, alloc
 *     /free calls, search-ns total, uptime — so the window does not "repeat"
 *     every tick; those are shown as derived rates but do not force a redraw).
...
static inline void nxmemstat_render(int win_id, const struct os1_sysstats *s,
                                    ...
                                    unsigned long dt_ms) {
  unsigned long mb = 4096UL / 1024UL; /* pages*4K -> KB; /1024 -> MB done below */
  (void)mb;
  ...
  unsigned long lcr_mb  = (unsigned long)((s->pmm_largest_contig_run * 4UL) / 1024UL);

  unsigned long csw_rate = 0, a_rate = 0, mean_srch = 0;
  if (prev && dt_ms > 0) {
    csw_rate = (unsigned long)((s->sched_ctx_switches - prev->sched_ctx_switches) * 1000UL / dt_ms);
    unsigned long da = (unsigned long)(s->pmm_alloc_calls - prev->pmm_alloc_calls);
    a_rate = da * 1000UL / dt_ms;
    if (da)
      mean_srch = (unsigned long)((s->pmm_alloc_search_ns_total - prev->pmm_alloc_search_ns_total) / da);
  }
  ...
  printf_win(win_id, "PMM    : free %lu / %lu MB   used %lu MB\n",
             free_mb, tot_mb, tot_mb - free_mb);
  printf_win(win_id, "frag   : largest %lu MB (%lu pg)   runs %lu\n",
             lcr_mb, (unsigned long)s->pmm_largest_contig_run,
  ...
  printf_win(win_id, "kheap  : in-use %lu KB  hi %lu KB  live %lu  pool %lu KB\n",
...
static inline int nxmemstat_render_if_changed(int win_id, struct os1_sysstats *prev,
  ...
  nxmemstat_render(win_id, &cur, had_prev ? prev : (struct os1_sysstats *)0, dt_ms);
  if (prev) *prev = cur;
  if (last_sig) *last_sig = sig;
  return 1;
}
...
static inline int nxmemstat_csv_line(char *buf, int size,
  ...
  return snprintf(buf, size,
    "MEMSTAT,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,0\n",
    t_s,
