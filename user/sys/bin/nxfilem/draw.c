...
void fm_draw_rect_outline(int x, int y, int w, int h, uint32_t color,
                          int thickness) {
  /* Top */
...
void fm_draw_centered_text(int x, int y, int w, int h, const char *text,
                           uint32_t color) {
  if (!text)
    return;
...
void fm_draw_file_icon(int x, int y, const fm_file_t *file) {
  if (file->is_dir) {
    ...
    if (ext) {
      ...
      } else if (strcmp(ext, ".tar") == 0 || strcmp(ext, ".zip") == 0 ||
                 strcmp(ext, ".gz") == 0) {
...
void fm_draw_scrollbar(int x, int y, int h, int total, int visible,
                       int offset) {
  if (total <= visible)
    return;
  ...
  fm_draw_rect(x - scrollbar_w, thumb_y, scrollbar_w, thumb_h,
               FM_COLOR_SURFACE2);
}
