 ...
 * produced by nxfont.  The on-disk format is:
 *
 *   [ struct font_header ]
...
static int font_validate_buffer(const void *data, size_t size) {
  ...
  if (h->num_chars == 0)
    return 0;
  if ((uint32_t)h->ascent + (uint32_t)h->descent == 0)
    return 0;

  if (mul_overflows_size(h->num_chars, sizeof(struct font_glyph_info)))
    return 0;
  size_t glyph_bytes = (size_t)h->num_chars * sizeof(struct font_glyph_info);
  if (add_overflows_size(sizeof(struct font_header), glyph_bytes))
    return 0;
  size_t bitmap_offset = sizeof(struct font_header) + glyph_bytes;
  if (add_overflows_size(bitmap_offset, h->bitmap_size))
    return 0;
  if (bitmap_offset + h->bitmap_size > size)
    return 0;

  const struct font_glyph_info *glyphs =
      (const struct font_glyph_info *)((const uint8_t *)data +
                                       sizeof(struct font_header));
  for (uint16_t i = 0; i < h->num_chars; i++) {
    const struct font_glyph_info *gi = &glyphs[i];
    if (mul_overflows_size(gi->width, gi->height))
      return 0;
    size_t glyph_size = (size_t)gi->width * (size_t)gi->height;
    if (add_overflows_size(gi->data_offset, glyph_size))
      return 0;
    if (gi->data_offset + glyph_size > h->bitmap_size)
      return 0;
  }
 ...
 *
 * path: filesystem path to a packed font produced by nxfont.
 *
 ...
 *   ctx->bitmap  = raw_data + sizeof(font_header) + num_chars *
 * sizeof(glyph_info)
 *
...
struct font_ctx *font_load(const char *path) {
  ...
  ctx->glyphs =
      (struct font_glyph_info *)((uint8_t *)data + sizeof(struct font_header));
  /* Bitmap bytes follow the glyph array. */
  ctx->bitmap =
      (uint8_t *)ctx->glyphs + h->num_chars * sizeof(struct font_glyph_info);
 ...
 */
static void draw_glyph(int win_id, struct font_ctx *ctx, int x, int y,
  ...
  for (int gy = 0; gy < gi->height; gy++) {
    ...
    while (gx < gi->width) {
      ...
      if (gx > span_start) {
        window_draw(win_id, start_x + span_start, start_y + gy, gx - span_start,
                    1, color);
      }
    }
  }
 ...
 */
void font_draw_string(int win_id, struct font_ctx *ctx, int x, int y,
