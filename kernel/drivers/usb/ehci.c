...
struct ehci {
  ...
  /* FIX(EHCI-MULTIEP-01): physical address (| LP_QH, or LP_T if none yet)
   * of the current head of the periodic (interrupt) schedule. Every
   * framelist[] slot always points here. See ehci_intr_open()'s comment
...
static inline uint32_t rd32(volatile uint8_t *p, uint32_t o) {
  return *(volatile uint32_t *)(p + o);
}
static inline void wr32(volatile uint8_t *p, uint32_t o, uint32_t v) {
  *(volatile uint32_t *)(p + o) = v;
}
...
static int ehci_run_ctrl(struct ehci *e, struct usb_device *dev,
                         struct ehci_qtd *last) {
  struct ehci_qh *qh = e->ctrl_qh;
  int eps = (dev->speed == USB_SPEED_HIGH)  ? 2
            : (dev->speed == USB_SPEED_LOW) ? 1
                                            : 0;
  int mp = dev->max_packet0 ? dev->max_packet0 : 8;
  qh->ep_char = (uint32_t)dev->addr | ((uint32_t)0 << 8) |
                ((uint32_t)eps << 12) | (1u << 14) /* DTC: toggle from qTD */ |
                ((uint32_t)mp << 16) |
  ...
  for (long s = 0; s < 60000000L; s++) {
    if (!(last->token & QTD_ACTIVE))
      break;
  }
  if (last->token & (QTD_ACTIVE | QTD_HALTED))
    return -1;
  return 0;
}
...
static int ehci_control(struct usb_hcd *hcd, struct usb_device *dev,
  ...
  if (len > 0) {
    if (!dir_in)
      memcpy((uint8_t *)e->ctrl_buf + 64, data, len);
    qtd_fill(&td[1], e->ctrl_qtd_phys + 2 * sizeof(struct ehci_qtd),
             dir_in ? QTD_PID_IN : QTD_PID_OUT, 1, len, e->ctrl_buf_phys + 64,
             0);
    n = 2;
  }
  ...
  if (ehci_run_ctrl(e, dev, &td[last]) < 0)
    return -1;
  ...
  if (dir_in && len > 0) {
    ...
    if (data && transferred > 0)
      memcpy(data, (uint8_t *)e->ctrl_buf + 64, transferred);
  }
  return transferred;
}
...
static int ehci_enumerate_port(struct usb_hcd *hcd, int port,
  ...
  for (long s = 0; s < 4000000L; s++) {
    if (!(rd32(e->op, EHCI_PORTSC(port)) & PORT_RESET))
      break;
  }
...
static int ehci_address(struct usb_hcd *hcd, struct usb_device *dev) {
  ...
  struct usb_setup s8 = {USB_DIR_IN, USB_REQ_GET_DESCRIPTOR,
                         (USB_DT_DEVICE << 8), 0, 8};
  if (ehci_control(hcd, dev, &s8, &dd, 8) < 8)
    return -1;
  ...
  if (ehci_control(hcd, dev, &sa, NULL, 0) < 0)
    return -1;
  dev->addr = new_addr;
...
static int ehci_intr_open(struct usb_hcd *hcd, struct usb_device *dev,
  ...
  (void)interval;
  struct ehci *e = hcd->priv;
  if (e->ep_count >= (int)(sizeof(e->eps) / sizeof(e->eps[0])))
    return -1;
  ...
  ep->qh->ep_char = (uint32_t)dev->addr | ((uint32_t)epn << 8) |
                    (2u << 12) /*HS*/ | (1u << 14) |
  ...
  /*
   * FIX(EHCI-MULTIEP-01): this used to unconditionally do
   *   ep->qh->horiz = LP_T;
   *   for (i = 0; i < 1024; i++) e->framelist[i] = link;
   * on EVERY call — i.e. every additional interrupt endpoint (a second
   * USB HID device: plug in both a keyboard AND a mouse on the same EHCI
   * controller, or a keyboard behind a hub whose OTHER downstream port
   * also has a device) completely REPLACED the periodic schedule instead
   * of joining it. The host controller only ever visits whatever the
   * framelist currently points to, so the PREVIOUSLY opened endpoint's QH
   * became structurally unreachable the moment a second one was opened —
   * it would simply stop receiving any polls, silently, with no error
   * anywhere (this driver has no IRQ path to notice a QH going idle).
   * uhci.c's ehci_intr_open() equivalent (uhci_intr_open) already does
   * this correctly by chaining each new QH to the previous one; this
   * mirrors that: the new QH's horiz points to whatever was the head
   * before (LP_T the very first time), and framelist[] is repointed at
   * the new head. This runs only when an interrupt endpoint is opened —
   * a handful of times during boot enumeration, never on a hot path — so
   * rewriting all 1024 framelist entries here is cheap. */
  ep->qh->horiz = e->periodic_head ? e->periodic_head : LP_T;
  e->periodic_head = (uint32_t)ep->qh_phys | LP_QH;
  for (int i = 0; i < 1024; i++)
    e->framelist[i] = e->periodic_head;
...
static int ehci_probe(struct hal_device *dev) {
  if (ehci_count >= (int)(sizeof(ehci_ctrls) / sizeof(ehci_ctrls[0])))
    return -1;
  ...
  for (long s = 0; s < 4000000L; s++)
    if (rd32(e->op, EHCI_USBSTS) & STS_HALTED)
      break;
  wr32(e->op, EHCI_USBCMD, CMD_HCRESET);
  for (long s = 0; s < 8000000L; s++)
    if (!(rd32(e->op, EHCI_USBCMD) & CMD_HCRESET))
      break;
  ...
  if (!e->async || !e->ctrl_qh || !e->ctrl_qtd || !e->ctrl_buf || !e->framelist)
    return -1;
...
void ehci_register(void) { driver_register(&ehci_driver); }