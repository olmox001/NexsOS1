...
| DRV-PCI-01 | W3 | MISSING | `kernel/include/drivers/pci.h:13`, `kernel/drivers/pci/pci.c` | `pci_scan_and_register()` is declared in the header but never defined or called. No callers exist in the tree. ***RESOLVED (#51)*** — now defined (`kernel/drivers/pci/pci.c:272`, arch-neutral ECAM/port scan into the HAL registry) and called on aarch64 (`kernel/arch/aarch64/hal.c:123`); amd64 uses its own virtio legacy/modern callback with firmware-programmed BARs. Boot logs show `PCI: scanning config space`. |
| DRV-UART-01 | W3 | BUG · SECURITY | `kernel/drivers/uart/pl011.c:109–118` | `uart_getc` and `uart_getc_nonblock` read `rx_tail` without a lock; `uart_irq_handler` writes `rx_head` under no lock; on SMP, two readers or a reader+IRQ can corrupt the index — requires a memory barrier at minimum, a lock for SMP correctness. ***RESOLVED (#52, `f4b1458`)*** — dedicated `rx_lock` (spinlock + IRQ save) wraps all rx_buf/rx_head/rx_tail accesses in producer + consumers; idle/WFI runs lock-released. Separate from TX `uart_lock`. 16550 (amd64) is polled/lock-free, unaffected. |
